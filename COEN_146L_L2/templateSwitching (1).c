...
* Description:  Calculates the difference between circuit
...
{
    double fact = -1;	// - Default the value of factorial to -1 (meaning error)
    //factorial function
    if (n < 0) // Negative number check
        printf("There is no factorial for a negative number.");
    else { // Valid number passed
		fact = 1;
		// - You will multiply the index by the current result until you reach n
        for (int i = 1; i <= n; ++i) {
            fact *= i;
        }
    }
